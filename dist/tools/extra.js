...
exports.sqliteQueryTool = {
    ...
    async execute(args, cwd) {
        try {
            ...
            if (!output)
                return { success: true, output: '(无结果)' };
        ...
        try {
            ...
            if (formatter === 'auto') {
                if (['.ts', '.tsx', '.js', '.jsx', '.css', '.json', '.md', '.html', '.yaml', '.yml'].includes(ext)) {
                    ...
                    return { success: false, output: '', error: `无法自动检测格式化工具（${ext}）` };
            }
            // 保存快照
            ...
            switch (formatter) {
                case 'prettier':
                    cmd = `npx prettier --write "${fullPath}"`;
                    break;
                case 'black':
                    cmd = `black "${fullPath}"`;
                    break;
                case 'gofmt':
                    cmd = `gofmt -w "${fullPath}"`;
                    break;
                case 'clang-format':
                    cmd = `clang-format -i "${fullPath}"`;
                    break;
