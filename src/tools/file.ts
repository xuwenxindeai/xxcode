...
async function readLineRange(fullPath: string, offset: number, limit: number): Promise<{ lines: string[]; hasMore: boolean }> {
  ...
  for await (const raw of rl) {
    ...
    if (lines.length >= limit) { hasMore = true; rl.close(); break; }
...
export const readTool: Tool = {
  ...
  async execute(args, cwd): Promise<ToolResult> {
    try {
      ...
      if (!fs.existsSync(fullPath)) return { success: false, output: '', error: `文件不存在: ${fullPath}` };
      const stat = fs.statSync(fullPath);
      if (stat.isDirectory()) return { success: false, output: '', error: `这是目录不是文件: ${fullPath}（用 list_dir 或 project_tree）` };
...
export const searchFilesTool: Tool = {
  ...
  async execute(args, cwd): Promise<ToolResult> {
    try {
      ...
      const files = await glob(args.pattern, { cwd, ignore: ['**/node_modules/**', '**/Pods/**', '**/.git/**', '**/build/**', '**/DerivedData/**', '**/dist/**'] });
      const shown = files.slice(0, MAX_RESULTS);
      let out = shown.join('\n') || '(无匹配文件)';
      if (files.length > MAX_RESULTS) out += `\n…(共 ${files.length} 个匹配，仅显示前 ${MAX_RESULTS} 个，请缩小 pattern)`;
