...
export const pythonReplTool: Tool = {
  ...
  async execute(args, cwd): Promise<ToolResult> {
    try {
      ...
      // 清理临时文件
      if (args.code) {
        try { fs.unlinkSync(path.join(cwd, '.tmp_agent_repl.py')); } catch {}
      }
...
export const pipTool: Tool = {
  ...
  async execute(args, _cwd): Promise<ToolResult> {
    try {
      ...
      switch (args.action) {
        case 'install':
          if (!args.package) return { success: false, output: '', error: '需要指定包名' };
          fullCmd += ` install ${args.package}${args.version || ''}`;
          if (args.upgrade) fullCmd += ' --upgrade';
          if (args.user) fullCmd += ' --user';
          break;
        case 'uninstall':
          if (!args.package) return { success: false, output: '', error: '需要指定包名' };
  ...
  async execute(args, cwd): Promise<ToolResult> {
    try {
      ...
      switch (args.action) {
        case 'install':
          cmd += ' install';
          if (args.package) cmd += ` ${args.package}`;
          if (args.dev) cmd += ' --save-dev';
          if (args.global) cmd += ' -g';
          break;
        case 'uninstall':
          if (!args.package) return { success: false, output: '', error: '需要指定包名' };
          ...
          break;
        case 'run':
          if (!args.script) return { success: false, output: '', error: '需要指定 script 名称' };
          cmd += ` run ${args.script}`;
          break;
        case 'list':
          cmd += ' ls --depth=0';
...
export const regexTool: Tool = {
  ...
  async execute(args): Promise<ToolResult> {
    try {
      ...
      const matches: Array<{ index: number; match: string; groups?: Record<string, string> }> = [];
      let match;
      ...
      if (isGlobal) {
        while ((match = regex.exec(args.text)) !== null && matches.length < (args.max_matches || 20)) {
          ...
          if (!match[0]) regex.lastIndex++; // 防止空匹配死循环
        }
      } else {
...
// 文件编码检测
export const detectEncodingTool: Tool = {
    ...
    try {
      ...
      switch (args.action) {
        ...
        case 'delete':
          ...
          if (deleteIdx < 0) return { success: false, output: `未找到 ${args.key}` };
        ...
        case 'list':
        default:
          if (!fs.existsSync(envPath)) return { success: true, output: `${envPath} 不存在` };
          const vars = lines
